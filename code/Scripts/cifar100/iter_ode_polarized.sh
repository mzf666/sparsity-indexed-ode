    ...
    for ((c = 0; c < ${#sparsities[@]}; c++)); do
      for ((i = 0; i < ${#models[@]}; i++)); do
        for ((j = 0; j < ${#prn_scopes[@]}; j++)); do
          ...
          --itr_lr 1e-2 --lr 3e-3 --free_conv1 \
          --score_option "mp" --mask_option "one" --sparsity_option "l2" --schedule "exp" \
          --mask_proc_option "qt" --mask_proc_eps 0.9 --mask_proc_ratio 0.9 --mask_proc_score_option "qt" --mask_proc_mxp
    ...
    for ((c = 0; c < ${#sparsities[@]}; c++)); do
      for ((i = 0; i < ${#models[@]}; i++)); do
        for ((j = 0; j < ${#prn_scopes[@]}; j++)); do
          ...
          --itr_lr 1e-2 --lr 3e-3 --free_conv1 \
          --score_option "mp" --mask_option "one" --sparsity_option "l2" --schedule "exp" \
          --mask_proc_option "ohm" --mask_proc_score_option "Id" --mask_proc_mxp
